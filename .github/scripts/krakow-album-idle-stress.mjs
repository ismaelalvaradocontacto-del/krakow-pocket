...
for(const [engineName,engine] of engines){
 ...
 try{
  ...
  const y1=await establishScroll(f);check('test establishes real non-zero scroll',y1>=80,String(y1));await resetCounts(page);await addMemory(page,'Cambio con scroll');await f.getByRole('heading',{name:'Cambio con scroll',exact:true}).waitFor({state:'visible',timeout:7000});await page.waitForTimeout(650);const y2=await evalFrame(f,()=>scrollY);c=await counts(page);check('non-zero scroll preserved after real refresh',y1>=80&&Math.abs(y2-y1)<=18,`${y1}->${y2}`);check('one data change causes one srcdoc regeneration',c.writes===1,JSON.stringify(c));

  await f.locator('body').hover();await page.mouse.wheel(0,-30000);await page.waitForTimeout(260);let top=await evalFrame(f,()=>scrollY);if(top>4){await page.mouse.wheel(0,-30000);await page.waitForTimeout(260);top=await evalFrame(f,()=>scrollY)}check('user can immediately override restoration and reach top',top<=4,String(top));await resetCounts(page);await addMemory(page,'Cambio desde arriba');await f.getByRole('heading',{name:'Cambio desde arriba',exact:true}).waitFor({state:'visible',timeout:7000});await page.waitForTimeout(650);const y0=await evalFrame(f,()=>scrollY);c=await counts(page);check('top remains top after refresh',y0<=6,String(y0));check('top refresh regenerates once',c.writes===1,JSON.stringify(c));

  await f.locator('#storyAlbum').click();await f.locator('#storyMode.open').waitFor();await resetCounts(page);await addMemory(page,'Mientras Historia');await page.waitForTimeout(1800);c=await counts(page);check('Story stays open during state change',await f.locator('#storyMode').evaluate(e=>e.classList.contains('open')));check('Story blocks album regeneration',c.writes===0,JSON.stringify(c));await f.locator('#storyMode .overlay-close').click();await f.getByRole('heading',{name:'Mientras Historia',exact:true}).waitFor({state:'visible',timeout:7000});await page.waitForTimeout(650);c=await counts(page);check('deferred Story refresh regenerates once',c.writes===1,JSON.stringify(c));

  await f.locator('.photo-button').first().click();await f.locator('#lightbox.open').waitFor();await resetCounts(page);await addMemory(page,'Mientras Lightbox');await page.waitForTimeout(1800);c=await counts(page);check('lightbox stays open during state change',await f.locator('#lightbox').evaluate(e=>e.classList.contains('open')));check('lightbox blocks album regeneration',c.writes===0,JSON.stringify(c));await page.keyboard.press('Escape');await f.getByRole('heading',{name:'Mientras Lightbox',exact:true}).waitFor({state:'visible',timeout:7000});await page.waitForTimeout(650);c=await counts(page);check('deferred lightbox refresh regenerates once',c.writes===1,JSON.stringify(c));

  await resetCounts(page);await addMemory(page,'Antes de PDF');const pdf=await page.evaluate(()=>window.KP_ALBUM_V5.print());check('PDF callable during pending state refresh',pdf===true);await page.waitForTimeout(1100);c=await counts(page);check('PDF lifecycle causes at most one regeneration',c.writes<=1,JSON.stringify(c));
