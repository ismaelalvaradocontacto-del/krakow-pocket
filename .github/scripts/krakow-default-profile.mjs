...
async function openSettings(page) {
  ...
  await page.waitForSelector('#settingsSheet[open]', { timeout: 3000 });
  return Date.now() - started;
}

async function closeSettings(page) {
  await page.locator('#closeSettings').click();
  await page.waitForFunction(() => !document.querySelector('#settingsSheet')?.open, { timeout: 3000 });
}
...
for (const [name, engine] of [['chromium', chromium], ['webkit', webkit]]) {
  ...
  await page.waitForSelector('#kpProfilePhotoManager', { timeout: 3000 });
  ...
  await page.waitForFunction(() => !!document.querySelector('#kpProfilePhotoPreview > svg.kp-profile-default'), { timeout: 3000 });
  ...
  await page.waitForFunction(() => localStorage.getItem('krakowPlayer') === 'Laura', { timeout: 3000 });
  await page.waitForFunction(() => (document.querySelector('#kpProfilePhotoTitle')?.textContent || '').includes('Laura'), { timeout: 3000 });
  ...
  await page.waitForFunction(() => localStorage.getItem('krakowPlayer') === 'Ismael', { timeout: 3000 });
  ...
  const ok = initial.compat === '2.2' && initial.photoModule === '2.0' && initial.bridge === '1.4' && initial.nativeDefault && initial.cssFallback && initial.protectedFallback && initial.persistentSettings && initial.noCompatObserver && initial.eventDriven && initial.noPolling && initial.count === 2 && initial.visible && initial.noBrokenDefaultImages &&
    maxOpenMs < 3000 && settings.pickerDefaults === 2 && settings.resetText.includes('Imagen por defecto') && settings.previewDefault && settings.closeVisible &&
