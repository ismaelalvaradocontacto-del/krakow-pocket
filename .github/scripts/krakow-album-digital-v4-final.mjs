...
for(const [name,engine] of engines){
 ...
 try{
  ...
  await page.waitForFunction(()=>KP_ALBUM_DIGITAL_V4?.version==='4.0'&&KP_ALBUM_DIGITAL_AMBIENT_FIX?.version==='1.1'&&KP_ALBUM_PHOTO_QUALITY?.version==='1.0'&&KP_ALBUM_EXPERIENCE?.digitalAlbum===true,{timeout:20000});
  ...
  const html=await page.evaluate(()=>KP_ALBUM_EXPERIENCE.html()),quality=await page.evaluate(()=>({max:KP_ALBUM_PHOTO_QUALITY.targetMax,size:KP_ALBUM_PHOTO_QUALITY.maxDataLength,safe:KP_ALBUM_PHOTO_QUALITY.storageSafeFallback}));
