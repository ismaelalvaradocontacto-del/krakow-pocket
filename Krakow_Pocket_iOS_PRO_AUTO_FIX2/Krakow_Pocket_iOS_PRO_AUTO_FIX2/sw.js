...
self.addEventListener("fetch",e=>{
  ...
  const runtimeCdn = u.hostname==="cdn.jsdelivr.net" && (
    u.pathname.includes("@paddleocr/paddleocr-js") ||
    u.pathname.includes("onnxruntime-web") ||
    u.pathname.includes("@techstark/opencv-js") ||
    u.pathname.includes("clipper-lib") ||
    u.pathname.includes("js-yaml") ||
    u.pathname.includes("pdfjs-dist@4.10.38") ||
    u.pathname.includes("tesseract.js@5") ||
    u.pathname.includes("tesseract.js-core") ||
    u.pathname.includes("@tesseract.js-data/") ||
    u.pathname.includes("leaflet@1.9.4") ||
    u.pathname.includes("pdf-lib@1.17.1")
  );
  const paddleModels = /(?:paddle|paddlex|bcebos)\./i.test(u.hostname) || u.hostname.endsWith("bcebos.com");
  if(runtimeCdn || paddleModels){
    e.respondWith(caches.match(e.request,{ignoreSearch:true}).then(hit=>hit||fetch(e.request).then(r=>{const x=r.clone();caches.open(CACHE).then(c=>c.put(e.request,x)).catch(()=>{});return r})));
    return;
  }
