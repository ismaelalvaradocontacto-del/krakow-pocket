...
function injectStyles(){
  ...
  style.textContent=`
    ...
    .kp-proof-head{display:flex;gap:12px;align-items:flex-start;justify-content:space-between}.kp-proof-head h2{margin:2px 0 3px;font-size:24px}.kp-proof-close{width:44px;height:44px;border-radius:14px;border:2px solid #6d432b;background:#fff0c7;font-size:21px;font-weight:900}
    ...
    .kp-proof-status{padding:12px 13px;border-radius:15px;background:#fff4d5;border:2px solid #d2aa6a;line-height:1.35;margin:10px 0}.kp-proof-status strong{display:block;margin-bottom:3px}
    .kp-proof-preview{display:none;margin:12px 0;border-radius:18px;overflow:hidden;border:3px solid #6d432b;background:#dccaa0}.kp-proof-preview img{display:block;width:100%;max-height:42dvh;object-fit:cover}
    ...
    @media(max-width:380px){.kp-proof-rule,.kp-proof-actions,.kp-album-actions{grid-template-columns:1fr}.kp-proof-actions #kpProofFinish{grid-column:auto}}
    @media(prefers-reduced-motion:reduce){.kp-album-entry,.kp-album-comment,.kp-album-cover .dragon{animation:none!important}}
  `;
...
function proofRefs(){
  return {
    place:document.getElementById("kpProofPlace"),
    distance:document.getElementById("kpProofDistance"),
    gpsChip:document.getElementById("kpProofGpsChip"),
    photoChip:document.getElementById("kpProofPhotoChip"),
    status:document.getElementById("kpProofStatus"),
    input:document.getElementById("kpProofInput"),
    label:document.getElementById("kpProofPhotoLabel"),
    preview:document.getElementById("kpProofPreview"),
    image:document.getElementById("kpProofImage"),
    retry:document.getElementById("kpProofRetry"),
...
function ensureProofDialog(){
  ...
  proofDialog.innerHTML=`<div class="kp-proof-shell">
    <div class="kp-proof-head"><div><div class="smart-kicker">MISIÓN VERIFICADA</div><h2 id="kpProofPlace">Cracovia</h2><div class="small">Para conseguir la escama hay que estar allí y guardar una foto.</div></div><button class="kp-proof-close" id="kpProofClose" aria-label="Cerrar">✕</button></div>
    <div class="kp-proof-rule"><div class="kp-proof-chip" id="kpProofGpsChip">📍 GPS pendiente</div><div class="kp-proof-chip" id="kpProofPhotoChip">📷 Foto pendiente</div></div>
    <div class="kp-proof-status" id="kpProofStatus"><strong>Comprobando ubicación…</strong>Espera un momento.</div>
    <div class="kp-proof-preview" id="kpProofPreview"><img id="kpProofImage" alt="Fotografía de la misión"></div>
    <div class="kp-proof-actions"><button id="kpProofRetry">↻ Comprobar GPS</button><label class="kp-proof-photo-label disabled" id="kpProofPhotoLabel">📷 Hacer foto<input id="kpProofInput" type="file" accept="image/*" capture="environment" disabled></label><button id="kpProofFinish" disabled>✓ Guardar foto y completar misión</button></div>
  </div>`;
...
async function checkGps(){
  ...
  const p=poi(activePoi),r=proofRefs(),limit=radius(activePoi);
  r.retry.disabled=true;r.status.innerHTML="<strong>Comprobando GPS…</strong>Buscando una posición precisa.";
  r.gpsChip.className="kp-proof-chip";r.gpsChip.textContent="📍 comprobando…";
  r.input.disabled=true;r.label.classList.add("disabled");r.label.classList.remove("ready");
  try{
    const pos=await getPosition();
    const dist=havMeters(pos,{lat:p.lat,lon:p.lon});
    activePosition={...pos,distance:dist};
    const ok=dist<=limit;
    r.distance.textContent="";
    r.gpsChip.className=`kp-proof-chip ${ok?"ok":"bad"}`;
    r.gpsChip.textContent=ok?`📍 ${Math.round(dist)} m · OK`:`📍 ${Math.round(dist)} m`;
    ...
    r.input.disabled=!ok;
    r.label.classList.toggle("disabled",!ok);r.label.classList.toggle("ready",ok);
    return ok;
  }catch(e){
    activePosition=null;
    r.gpsChip.className="kp-proof-chip bad";r.gpsChip.textContent="📍 GPS no disponible";
...
function canvasData(img,max,quality){
...
async function optimizePhoto(file){
  ...
  let data=canvasData(img,720,.7);
  if(data.length>180000)data=canvasData(img,560,.62);
  if(data.length>180000)data=canvasData(img,440,.56);
  return data;
}
...
async function onPhotoSelected(e){
  ...
  const r=proofRefs();
  r.photoChip.className="kp-proof-chip";r.photoChip.textContent="📷 preparando…";
  r.status.innerHTML="<strong>Preparando la foto…</strong>La reducimos para que el álbum no pese demasiado.";
  try{
    const gpsOk=await checkGps();
    if(!gpsOk){e.target.value="";return}
    const dataUrl=await optimizePhoto(file);
    r.image.src=dataUrl;r.preview.style.display="block";
    r.photoChip.className="kp-proof-chip ok";r.photoChip.textContent="📷 Foto lista";
    r.status.innerHTML=`<strong>✓ Evidencia preparada</strong>${esc(currentPlayer())}, esta foto se guardará con la misión. El GPS queda registrado solo como distancia verificada, no como coordenadas.`;
    r.finish.dataset.photo=dataUrl;r.finish.disabled=false;r.finish.style.display="flex";
  }catch{
    r.photoChip.className="kp-proof-chip bad";r.photoChip.textContent="📷 No se pudo leer";
    r.status.innerHTML="<strong>No he podido preparar esa imagen</strong>Haz otra foto o elige otra de la fototeca.";
    e.target.value="";
  }
}
...
function openProof(id){
  ...
  const s=read(),old=evidence(s)[id];
  if(old?.photo&&old?.verified){allowAndComplete(id);return true}
  activePoi=id;activePosition=null;
  ensureProofDialog();
  const r=proofRefs();
  r.place.textContent=p.name;r.distance.textContent="";r.gpsChip.className="kp-proof-chip";r.gpsChip.textContent="📍 GPS pendiente";r.photoChip.className="kp-proof-chip";r.photoChip.textContent="📷 Foto pendiente";
...
function finishMission(){
  const r=proofRefs(),photo=r.finish.dataset.photo,id=activePoi,p=poi(id),q=quest(id);
  ...
  const entry={
    id,questId:q.id,place:p.name,title:q.title,photo,comment:COMMENTS[id]||`Una misión menos y un recuerdo más de ${p.name}.`,by:currentPlayer(),completedAt:now(),distance:Math.round(dist),radius:limit,verified:true,updatedAt:now()
  };
  try{setStateEvidence(id,entry)}catch(e){
    r.status.innerHTML="<strong>No queda espacio suficiente en el iPhone</strong>No cierres esta pantalla. Prueba con otra foto; la app la comprimirá de nuevo.";return
  }
  proofDialog.close();
  allowAndComplete(id);
}
...
function allowAndComplete(id){
  ...
  if(b){b.click();setTimeout(()=>{allowId=null;renderAlbumCard()},60)}
}

function interceptMissionCompletion(e){
...
function formatDate(iso){
  try{return new Intl.DateTimeFormat("es-ES",{weekday:"short",day:"numeric",month:"short",hour:"2-digit",minute:"2-digit"}).format(new Date(iso))}catch{return""}
}

function albumEntries(){
  const e=evidence(read());
  return D.quests.map(q=>e[q.poi]).filter(x=>x?.photo&&x?.verified).sort((a,b)=>new Date(a.completedAt)-new Date(b.completedAt));
}

function albumMarkup(entries,{standalone=false}={}){
  if(!entries.length)return `<div class="kp-album-empty"><div style="font-size:52px">📷</div><h3>El álbum aún está vacío</h3><p>Las fotos aparecerán aquí a medida que completéis misiones verificadas.</p></div>`;
  return `<div class="kp-album-grid">${entries.map((x,i)=>`<article class="kp-album-entry" style="animation-delay:${Math.min(i*.08,.6)}s"><img src="${x.photo}" alt="${esc(x.place)}"><div class="kp-album-copy"><div class="kp-album-meta">${esc(formatDate(x.completedAt))} · ${esc(x.by||"Ambos")} · GPS ${Math.round(x.distance||0)} m</div><h3>${esc(x.title||x.place)}</h3><div>${esc(x.place)}</div><div class="kp-album-comment">🐉 ${esc(x.comment||COMMENTS[x.id]||"")}</div></div></article>`).join("")}</div>`;
}

function ensureAlbumDialog(){
  if(albumDialog)return albumDialog;
  albumDialog=document.createElement("dialog");albumDialog.id="kpAlbumDialog";
  albumDialog.innerHTML=`<div class="kp-album-shell"><div class="kp-album-top"><div><div class="smart-kicker">CRÓNICA FOTOGRÁFICA</div><h2>Álbum de Cracovia</h2></div><button class="kp-album-close" id="kpAlbumClose" aria-label="Cerrar">✕</button></div><div id="kpAlbumContent"></div><button class="btn green kp-album-download" id="kpAlbumDownloadInside">⬇ Descargar álbum</button></div>`;
  document.body.appendChild(albumDialog);
  document.getElementById("kpAlbumClose").onclick=()=>albumDialog.close();
  document.getElementById("kpAlbumDownloadInside").onclick=downloadAlbum;
  return albumDialog;
}

function openAlbum(){
  ensureAlbumDialog();const entries=albumEntries(),s=read();
  document.getElementById("kpAlbumContent").innerHTML=`<section class="kp-album-cover"><div class="dragon">🐉</div><h1>Kraków Pocket</h1><p><strong>Ismael + Laura · Cracovia</strong></p><p>${entries.length} fotografías verificadas · ${completed(s)}/${D.quests.length} misiones completadas</p></section>${albumMarkup(entries)}`;
  if(!albumDialog.open)albumDialog.showModal();
}

function standaloneAlbumHtml(entries){
  const s=read(),title=`Kraków Pocket · Álbum de Cracovia`;
...
function renderAlbumCard(){
  const diary=document.getElementById("diary");if(!diary)return;
  let card=document.getElementById("kpAlbumCard");
  if(!card){
    card=document.createElement("article");card.id="kpAlbumCard";card.className="card";
    const segment=diary.querySelector(".segment");diary.insertBefore(card,segment||diary.firstChild);
    card.addEventListener("click",e=>{if(e.target.closest("#kpAlbumOpen"))openAlbum();if(e.target.closest("#kpAlbumDownload"))downloadAlbum()});
  }
  const entries=albumEntries(),n=completed(read());
  card.innerHTML=`<div class="row start"><div><div class="smart-kicker">RECUERDO DE LA AVENTURA</div><h2>📸 Álbum de Cracovia</h2></div><span class="pill ${entries.length===D.quests.length?"green":"gold"}">${entries.length}/${D.quests.length} fotos</span></div><p class="small">Cada misión verificada añade su fotografía, quién la hizo, la distancia GPS y un comentario del Dragón. ${n===D.quests.length?"🏆 La aventura está completa: vuestro álbum final ya está listo.":"Podéis verlo y descargarlo en cualquier momento."}</p><div class="kp-album-actions"><button class="btn secondary" id="kpAlbumOpen">👀 Ver álbum</button><button class="btn" id="kpAlbumDownload" ${entries.length?"":"disabled"}>⬇ Descargar</button></div>`;
}

function expose(){
  window.KP_MISSION_PROOF={version:VERSION,requiresPhoto:true,requiresProximity:true,sharedEvidence:true,albumDownload:true,animatedComments:true,radii:{...RADII},open:openProof,openAlbum,downloadAlbum,getEvidence:()=>evidence(read()),count:()=>albumEntries().length};
}

function boot(){
  injectStyles();ensureProofDialog();ensureAlbumDialog();renderAlbumCard();expose();
  document.addEventListener("click",interceptMissionCompletion,true);
  window.addEventListener("kp:mission-evidence-sync",renderAlbumCard);
  window.addEventListener("kp:mission-evidence-local",renderAlbumCard);
  window.addEventListener("pageshow",()=>setTimeout(renderAlbumCard,250),{passive:true});
  document.addEventListener("visibilitychange",()=>{if(!document.hidden)setTimeout(renderAlbumCard,250)},{passive:true});
}
