...
/* En iPhone, abrir Google Maps en una pestaña nueva podía dejar una pestaña blanca al volver.
   Lanzamos la app de Google Maps directamente y solo usamos la web como respaldo. */
function googleMapsScheme(raw){
  try{
    const u=new URL(raw,location.href);
    if(!/google\.(com|[a-z.]+)\/maps|maps\.google\./i.test(u.hostname+u.pathname))return null;
    const q=u.searchParams.get("query")||u.searchParams.get("q");
    if(q)return`comgooglemaps://?q=${encodeURIComponent(q)}`;
    const origin=u.searchParams.get("origin"),destination=u.searchParams.get("destination"),mode=u.searchParams.get("travelmode")||"walking";
    if(destination)return`comgooglemaps://?saddr=${encodeURIComponent(origin||"")}&daddr=${encodeURIComponent(destination)}&directionsmode=${encodeURIComponent(mode)}`;
  }catch{}
  return null;
}
function openMapsSafe(raw){
  const url=String(raw||"");
  const scheme=googleMapsScheme(url);
  if(!scheme){location.href=url;return null}
  let leftPage=false;
  const vis=()=>{if(document.hidden)leftPage=true};
  document.addEventListener("visibilitychange",vis);
  location.href=scheme;
  setTimeout(()=>{
    document.removeEventListener("visibilitychange",vis);
    if(!leftPage&&document.visibilityState==="visible")location.href=url;
  },900);
  return null;
}
...
window.open=function(url,target,features){
  const s=String(url||"");
  if(/google\.(com|[a-z.]+)\/maps|maps\.google\./i.test(s))return openMapsSafe(s);
...
function ensure(){
  ...
  if(!$("#kpStreetBar")){const e=document.createElement("div");e.id="kpStreetBar";e.className="kp-street-bar";e.innerHTML='<span class="kp-street-label">☀️ Modo calle</span><button id="kpBackBase">🏠 Base</button><button id="kpBackMda">🚌 MDA</button>';document.body.appendChild(e)}
}
...
function render(){ensure();renderActivity();wireBar();const ver=$("#settingsVersion");if(ver)ver.textContent="Kraków Pocket · v3.5.1"}
