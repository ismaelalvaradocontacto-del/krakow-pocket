...
function closestAnchor(pos){return EXTRA.anchors.map(a=>({...a,distance:havMeters(pos,a)})).sort((a,b)=>a.distance-b.distance)[0]}
function evidence(){return read().missionEvidence?.[ID]||null}
function verified(){const e=evidence();return!!(e?.verified&&e?.photo)}

function injectStyles(){
  if(document.querySelector('style[data-kp-auschwitz-extra="2"]'))return;
  document.querySelector('style[data-kp-auschwitz-extra="1"]')?.remove();
  const s=document.createElement("style");s.dataset.kpAuschwitzExtra="2";s.textContent=`
  ...
  #kpAuschwitzExtraCard .kp-extra-status{display:inline-flex;align-items:center;gap:5px;padding:7px 10px;border:2px solid #988878;border-radius:999px;background:#f7f0e4;font-size:12px;font-weight:900}.kp-extra-status.done{background:#dfe8d5!important;border-color:#71815f!important}
  #kpAuschwitzDialog{border:0;padding:0;background:transparent;max-width:none;max-height:none;width:100%;height:100%;margin:0}
  #kpAuschwitzDialog::backdrop{background:rgba(34,30,27,.7);backdrop-filter:blur(2px);-webkit-backdrop-filter:blur(2px)}
...
function ensureDialog(){
  ...
  dialog.innerHTML=`<div class="kp-auschwitz-shell"><div class="kp-auschwitz-head"><div><div class="kp-extra-kicker">VISITA EXTRA · MEMORIA</div><h2>Auschwitz-Birkenau</h2><div class="small">No suma puntos. Solo verifica que habéis estado allí y guarda un recuerdo en el álbum digital.</div></div><button class="kp-auschwitz-close" id="kpAuschwitzClose" type="button" aria-label="Cerrar">✕</button></div><div class="kp-auschwitz-rule"><div class="kp-auschwitz-chip" id="kpAuschwitzGps">📍 GPS pendiente</div><div class="kp-auschwitz-chip" id="kpAuschwitzPhotoChip">📷 Foto pendiente</div></div><div class="kp-auschwitz-status" id="kpAuschwitzStatus"><strong>Comprobando ubicación…</strong>Se acepta Auschwitz I o Auschwitz II-Birkenau.</div><div class="kp-auschwitz-preview" id="kpAuschwitzPreview"><img id="kpAuschwitzImage" alt="Recuerdo de Auschwitz-Birkenau"></div><div class="kp-auschwitz-actions"><button id="kpAuschwitzRetry" type="button">↻ Comprobar GPS</button><label class="kp-auschwitz-photo disabled" id="kpAuschwitzPhotoLabel">📷 Hacer foto<input id="kpAuschwitzInput" type="file" accept="image/*" capture="environment" disabled></label><button id="kpAuschwitzSave" type="button" disabled>✓ Guardar en el álbum</button></div><p class="small" style="margin:12px 2px 0;color:#66584d">La app guarda la distancia verificada, no vuestras coordenadas exactas.</p></div>`;
...
async function checkGps(){
  ...
  try{
    const pos=await getPosition(),a=closestAnchor(pos),ok=a.distance<=a.radius;activePos={...pos,site:a.name,distance:a.distance,radius:a.radius};
    r.gps.className=`kp-auschwitz-chip ${ok?"ok":"bad"}`;r.gps.textContent=ok?`📍 ${Math.round(a.distance)} m · OK`:`📍 ${Math.round(a.distance)} m`;
    r.status.innerHTML=ok?`<strong>✓ Ubicación verificada</strong>Estáis en ${esc(a.name)}. Podéis guardar una fotografía respetuosa de la visita.`:`<strong>Aún no estáis en el Memorial</strong>La zona válida más cercana es ${esc(a.name)}. La foto se desbloqueará cuando lleguéis.`;
    r.input.disabled=!ok;r.label.classList.toggle("disabled",!ok);r.label.classList.toggle("ready",ok);return ok;
  }catch{
    activePos=null;r.gps.className="kp-auschwitz-chip bad";r.gps.textContent="📍 GPS no disponible";r.status.innerHTML="<strong>No puedo verificar la ubicación</strong>Comprueba el permiso de ubicación y vuelve a intentarlo.";return false;
...
async function photoSelected(e){
  const file=e.target.files?.[0],r=refs();if(!file)return;
  if(!(await checkGps())){e.target.value="";return}
  try{
    const img=await loadImage(file),data=jpeg(img);r.image.src=data;r.preview.style.display="block";r.photo.className="kp-auschwitz-chip ok";r.photo.textContent="📷 Foto lista";r.status.innerHTML="<strong>✓ Recuerdo preparado</strong>Se guardará como visita extra de memoria, sin puntos ni escamas.";r.save.dataset.photo=data;r.save.disabled=false;r.save.style.display="flex";
  }catch{r.photo.className="kp-auschwitz-chip bad";r.photo.textContent="📷 No se pudo leer";r.status.innerHTML="<strong>No he podido preparar la foto</strong>Prueba con otra imagen.";e.target.value=""}
}
...
function saveVisit(){
  ...
  const stamp=now(),s=read(),entry={id:ID,bonus:true,extra:true,album:true,verified:true,place:EXTRA.place,title:"Visita a Auschwitz-Birkenau",photo,comment:EXTRA.comment,by:player(),completedAt:stamp,distance:Math.round(activePos.distance),radius:activePos.radius,site:activePos.site,updatedAt:stamp};
  ...
  try{window.dispatchEvent(new CustomEvent("kp:statechange",{detail:{source:"auschwitz-extra",id:ID}}))}catch{}
  nudgeSync();dialog.close();renderCard();
...
function bind(){
  injectStyles();ensureDialog();renderCard();
  window.addEventListener("kp:mission-evidence-sync",renderCard);
  window.addEventListener("kp:mission-evidence-local",renderCard);
  window.addEventListener("pageshow",()=>setTimeout(renderCard,100),{passive:true});
  window.KP_AUSCHWITZ_EXTRA={version:VERSION,optional:true,countsTowardCore:false,photoMemory:true,respectMode:true,acceptsAuschwitzI:true,acceptsBirkenau:true,storesExactCoordinates:false,albumUnifiedV5:true,legacyAlbumExporter:false,open:openDialog,isSaved:verified,config:EXTRA};
}
